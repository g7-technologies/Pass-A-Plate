...
export default class WritetoUs extends React.Component {
  ...
  set_token = async() =>{
    var tokken = await AsyncStorage.getItem('email')
  ...
  message_Request = () =>{
    ...
    formData.append('email', this.state.token);
    formData.append('subject', this.state.title);
    formData.append('message', this.state.message);
    try{
      fetch(`${BasePath}write-to-us`, {
        method: 'POST',
        headers: {
       ...
       {/*---------------Head Section---------------*/}
        <View style={{height:'20%',backgroundColor:Color.backgroundcolor,flexDirection:'row',borderBottomLeftRadius:30,}}>
          ...
          <View style={{justifyContent:'center',flex:1}}>          
            <Text style= {{textAlign:'center',fontSize:Font.headingText,fontFamily:Font.AgencyBold,color:Color.HeadingColor}}>Message</Text>
          </View>
        </View>
          ...
          <View style={{flex:1,padding:10,justifyContent:'center',backgroundColor:'#cdcccc',borderBottomRightRadius:30,borderTopRightRadius:30,marginRight:20,marginTop:20}}>
            ...
            {this.state.isLoading?
              ...
              <TouchableOpacity onPress={()=>this.send_message()} style={{width:'85%',borderWidth:1,marginTop:15,borderRadius:10,marginLeft:30,borderColor:'#044355'}}>
                <Text style={{textAlign:'center',padding:10,fontFamily:'Agency',color:'#044355',fontSize:25}}>Send</Text>
              </TouchableOpacity>
            }
        ...
        <View style={{flex:0.2,backgroundColor:Color.backgroundcolor,justifyContent:'center',alignItems:'center',borderTopLeftRadius:30,borderBottomLeftRadius:30,marginTop:10,marginLeft:20}}>
          <Text style={{color:Color.HeadingColor,textAlign:'center',marginTop:10,fontFamily:Font.Agency,fontSize:Font.normalText}}>Give Us A Call</Text>
            <TouchableOpacity onPress={()=>this.dialCall()} style={{flexDirection:'row'}}>
              <Ionicons name="md-call" color='#fff' size={20} style={{marginTop:12,marginRight:5}}/>
              <Text style={{color:'#fff',textAlign:'center',marginTop:10,fontSize:Font.normalText,fontFamily:Font.AgencyBold,}}>12345678900</Text>
            </TouchableOpacity>
        </View>
      </View>
    );
