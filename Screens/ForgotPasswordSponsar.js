...
export default class ForgotPasswordSponsar extends Component{
  ...
  reset_password = () =>{
    ...
    try{
      ...
      fetch(`${BasePath}sponsor_forgot_password.php`, {
        method: 'POST',
        headers: {
    ...
    if(this.state.fontLoaded){
      ...
      <View style={{flex:1,padding:20,backgroundColor:'#fff'}}> 
        ...
        <KeyboardAvoidingView style={{flex:1,}} behavior='padding' enabled>
          <TextInput placeholder='Email' placeholderTextColor={Color.PlaceholderColor} onChangeText={value=>this.setState({email:value})} value={this.state.email} style={{width:'100%',fontFamily:Font.AgencyBold,marginTop:5,borderWidth:1,borderRadius:5,borderColor:Color.bordercolor,padding:5,paddingHorizontal:15,marginBottom:10,fontSize:Font.normalText}} />
